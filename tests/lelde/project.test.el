...
(describe "lelde/project empty project (only .gitignore)"
  (let* ((project-path nil)
    ...
    (it "assertions"
      ...
      (should (string= (plist-get pinfo :tests-dir) "tests"))
      (should (string= (plist-get pinfo :tests-path)
                       (f-expand "tests" project-path)))
      (should (string= (plist-get pinfo :test-lib-dir) "tests/lib"))
      (should (string= (plist-get pinfo :test-lib-path)
                       (f-expand "tests/lib" project-path)))
      (should (string= (plist-get pinfo :test-rsc-dir) "tests/rsc"))
      (should (string= (plist-get pinfo :test-rsc-path)
                       (f-expand "tests/rsc" project-path)))
      )))
